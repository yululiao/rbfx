...
/// Locate the engine source checkout that corresponds to the binaries a platform builds against.
...
ea::string FindEngineCheckout(FileSystem* fs, const BuildPlatformData& platform, ea::vector<ea::string>& errors)
{
    ea::string candidate = RemoveTrailingSlash(ForwardSlashes(platform.engineBin_));
    ...
    errors.push_back(Format("Could not find an engine source checkout above '{}'. The Android project needs "
        ...
        "directory that lives inside the checkout.", platform.engineBin_));
    return EMPTY_STRING;
}
...
ea::string BuildGradleText(const BuildPlatformData& platform, const ea::string& engineCheckout,
    const ea::string& projectName)
{
    const AndroidBuildSettings& android = platform.android_;
    ...
    AppendLine(text, "// survive belongs in Build.json under the platform named " + projectName + ".");
    AppendLine(text, "//");
    ...
    AppendLine(text, "        ndk {");
    AppendLine(text, "            // From the platform, because a build for an ABI the engine was not configured");
    ...
    if (platform.encryptScripts_)
    {
        ...
        AppendLine(text, "                def scriptKey = System.getenv(" +
            GroovyString(platform.scriptKeyEnvVar_) + ")");
    ...
    AppendLine(text, "        // the build pipeline staged and packed for this exact platform.");
...
ea::string ManifestText(const BuildPlatformData& platform)
{
    ea::string text;
    ...
    AppendLine(text, "            android:screenOrientation=\"" + ForwardSlashes(platform.android_.orientation_) + "\"");
...
ea::string BuildScriptText(const ea::string& platformName)
{
    ea::string text;
    AppendLine(text, "# Generated by the rbfx editor build (platform " + platformName + ").");
    ...
    const ea::string& outputDir, const ea::string& resourceDir, ea::vector<ea::string>& errors)
{
    ...
    {
        errors.push_back("The Android project can only be generated for an Android platform.");
        return false;
    }
    ...
    const ea::string engineCheckout = FindEngineCheckout(fs, platform, errors);
    ...
    // A platform that names an icon replaces the template one; the manifest refers to it by resource
    // name, so the file goes into the density independent bucket and every screen finds it.
    if (!platform.android_.icon_.empty())
    {
        const ea::string icon = ForwardSlashes(platform.android_.icon_);
        if (!fs->FileExists(icon))
        {
            errors.push_back(Format("The launcher icon named in the platform does not exist: '{}'.", icon));
        }
    ...
    const ea::string dataResource = platform.packData_ ? "Data.pak" : "Data/";
    const ea::string coreDataResource = platform.packData_ ? "CoreData.pak" : "CoreData/";
    ...
    ok &= WriteText(context, output + "build.gradle", BuildGradleText(platform, engineCheckout, projectName), errors);
    ok &= WriteText(context, output + "AndroidManifest.xml", ManifestText(platform), errors);
    ...
    ok &= WriteText(context, output + "build_android.ps1", BuildScriptText(platform.name_), errors);
    ...
    if (ok)
    {
        URHO3D_LOGINFO("[Build] Android project for '{}' written to {} (engine checkout {}, application id {})",
            platform.name_, output, engineCheckout, platform.android_.applicationId_);
