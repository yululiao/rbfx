...
void RegisterNetworkBindings(sol::state& lua, Context* context)
{
    ...
    {
        using LUA_THIS = Network;
        LUA_CLASS(Network, sol::no_constructor
            LUA_BASES(Object)
            LUA_MEMBER_FUNC_RAW(Connect, [](Network* network, const char* url, Scene* scene) -> bool {
                return network && network->Connect(URL(url), scene);
            })
            LUA_MEMBER_FUNC_RAW(StartServer, [](Network* network, unsigned short port) -> bool {
                return network && network->StartServer(URL(port));
            })
            LUA_MEMBER_FUNC_RAW(Disconnect, [](Network* network) {
                if (network)
                    network->Disconnect();
            })
            LUA_MEMBER_FUNC(GetServerConnection)
            LUA_MEMBER_FUNC_RAW(GetClientConnections, [](Network* network, sol::this_state s) -> sol::table {
            ...
            LUA_MEMBER_FUNC(SetUpdateFps)
            LUA_MEMBER_FUNC_RAW(RegisterRemoteEvent, [](Network* network, const char* eventName) {
                if (network)
                    network->RegisterRemoteEvent(StringHash(eventName));
            })
            LUA_MEMBER_FUNC(IsServerRunning)
            LUA_MEMBER_FUNC_RAW(StopServer, [](Network* network) {
                if (network)
                    network->StopServer();
            })
            // Broadcast a VectorBuffer message to all clients (16_Chat).
            LUA_MEMBER_FUNC_RAW(BroadcastMessage, [](Network* network, int messageId, VectorBuffer* msg) {
                if (network && msg)
    ...
    {
        using LUA_THIS = Connection;
        LUA_CLASS(Connection, sol::no_constructor
            LUA_BASES(Object)
            LUA_MEMBER_FUNC_RAW(Disconnect, static_cast<void (Connection::*)()>(&Connection::Disconnect))
            LUA_MEMBER_FUNC(GetScene)
            LUA_MEMBER_FUNC(SetScene)
            LUA_MEMBER_FUNC(IsConnected)
            LUA_MEMBER_FUNC(GetAddress)
            // Traffic statistics for the overlay UI (17_SceneReplication).
            LUA_MEMBER_FUNC(GetPacketsInPerSec)
            LUA_MEMBER_FUNC(GetPacketsOutPerSec)
            LUA_MEMBER_FUNC(GetBytesInPerSec)
            LUA_MEMBER_FUNC(GetBytesOutPerSec)
            // Send a VectorBuffer message as reliable + ordered (16_Chat).
            LUA_MEMBER_FUNC_RAW(SendMessage, [](Connection* connection, int messageId, VectorBuffer* msg) {
                if (connection && msg)
                    connection->SendMessage(static_cast<NetworkMessageId>(messageId), *msg);
            })
            LUA_MEMBER_FUNC(ToString)
        );
    }
    ...
    {
        using LUA_THIS = VectorBuffer;
        LUA_CLASS(VectorBuffer,
            sol::call_constructor, sol::factories([]() {
                return VectorBuffer();
            })
            LUA_MEMBER_FUNC_RAW(WriteString, [](VectorBuffer* buffer, const char* text) {
                if (buffer)
                    buffer->WriteString(text);
            })
            LUA_MEMBER_FUNC_RAW(ReadString, [](VectorBuffer* buffer) {
                return buffer ? buffer->ReadString() : ea::string();
            })
            LUA_MEMBER_FUNC_RAW(WriteFloat, [](VectorBuffer* buffer, float value) {
                if (buffer)
                    buffer->WriteFloat(value);
            })
            LUA_MEMBER_FUNC_RAW(ReadFloat, [](VectorBuffer* buffer) {
                return buffer ? buffer->ReadFloat() : 0.0f;
            })
            LUA_MEMBER_FUNC_RAW(WriteVLE, [](VectorBuffer* buffer, unsigned value) {
                if (buffer)
                    buffer->WriteVLE(value);
            })
            LUA_MEMBER_FUNC_RAW(ReadVLE, [](VectorBuffer* buffer) {
                return buffer ? buffer->ReadVLE() : 0u;
            })
            // Rewind before re-reading an in-memory scene snapshot
            // (49_Urho2DIsometricDemo reload).
            LUA_MEMBER_FUNC_RAW(Seek, [](VectorBuffer* buffer, unsigned position) {
                return buffer ? buffer->Seek(position) : 0u;
            })
            LUA_MEMBER_FUNC_RAW(GetSize, [](VectorBuffer* buffer) {
                return buffer ? buffer->GetSize() : 0u;
            })
            LUA_MEMBER_FUNC_RAW(Clear, [](VectorBuffer* buffer) {
                if (buffer)
                    buffer->Clear();
            })
        );
    ...
    {
        using LUA_THIS = LuaMemoryBuffer;
        lua.new_usertype<LuaMemoryBuffer>("MemoryBuffer",
            ...
            LUA_MEMBER_FUNC_RAW(ReadString, [](LuaMemoryBuffer* buffer) {
                return buffer ? buffer->buffer_.ReadString() : ea::string();
            })
            LUA_MEMBER_FUNC_RAW(ReadFloat, [](LuaMemoryBuffer* buffer) {
                return buffer ? buffer->buffer_.ReadFloat() : 0.0f;
            })
            LUA_MEMBER_FUNC_RAW(ReadVLE, [](LuaMemoryBuffer* buffer) {
                return buffer ? buffer->buffer_.ReadVLE() : 0u;
            })
            // Read a packed Vector3, used to walk physics contact data
            // (18_CharacterDemo).
            LUA_MEMBER_FUNC_RAW(ReadVector3, [](LuaMemoryBuffer* buffer) {
                return buffer ? buffer->buffer_.ReadVector3() : Vector3::ZERO;
            })
            LUA_MEMBER_FUNC_RAW(IsEof, [](LuaMemoryBuffer* buffer) {
    ...
    {
        using LUA_THIS = ReplicationManager;
        LUA_CLASS(ReplicationManager, sol::no_constructor
            LUA_BASES(Component, Serializable, Object)
            LUA_MEMBER_FUNC(GetClientReplica)
        );
    }
    ...
    {
        using LUA_THIS = ClientReplica;
        LUA_CLASS(ClientReplica, sol::no_constructor
            LUA_MEMBER_FUNC(GetOwnedNetworkObject)
        );
    }

    // NetworkObject: replicated object component base.
    {
        using LUA_THIS = NetworkObject;
        LUA_CLASS(NetworkObject, sol::no_constructor
            LUA_BASES(Component, Serializable, Object)
            LUA_MEMBER_FUNC_RAW(GetNode, &Component::GetNode)
        );
    }
    ...
    {
        using LUA_THIS = BehaviorNetworkObject;
        LUA_CLASS(BehaviorNetworkObject, sol::no_constructor
            LUA_BASES(NetworkObject, Component, Serializable, Object)
            LUA_MEMBER_FUNC_RAW(SetClientPrefab, &StaticNetworkObject::SetClientPrefab)
            LUA_MEMBER_FUNC_RAW(SetOwner, [](BehaviorNetworkObject* object, Connection* owner) {
                if (object)
    ...
    {
        using LUA_THIS = HttpRequest;
        LUA_CLASS(HttpRequest,
            sol::call_constructor, sol::factories(
            ...
            LUA_MEMBER_FUNC(GetError)
            LUA_MEMBER_FUNC_RAW(GetState, [](HttpRequest* request) {
                return request ? static_cast<int>(request->GetState()) : 0;
            })
            LUA_MEMBER_FUNC(GetStatusCode)
            LUA_MEMBER_FUNC_RAW(ReadString, [](HttpRequest* request) {
                return request ? request->ReadString() : ea::string{};
            })
            LUA_MEMBER_FUNC_RAW(IsEof, [](HttpRequest* request) {
    ...
    {
        using LUA_THIS = JSONFile;
        LUA_CLASS(JSONFile,
            sol::call_constructor, sol::factories(
                [context]() { return SharedPtr<JSONFile>(new JSONFile(context)); })
            LUA_BASES(Resource, Object)
            LUA_MEMBER_FUNC_RAW(FromString, [](JSONFile* file, const char* json) {
                return file && file->FromString(json);
            })
            LUA_MEMBER_FUNC_RAW(GetRoot, [](JSONFile* file) -> JSONValue {
    ...
    {
        using LUA_THIS = JSONValue;
        LUA_CLASS(JSONValue,
            sol::call_constructor, sol::factories([]() { return JSONValue{}; })
            LUA_MEMBER_FUNC_OVERLOAD(Get,
            ...
            LUA_MEMBER_FUNC_RAW(GetString, [](const JSONValue* value) {
                return value ? value->GetString() : ea::string{};
            })
            LUA_MEMBER_FUNC(GetInt)
            LUA_MEMBER_FUNC(GetBool)
            LUA_MEMBER_FUNC(GetDouble)
            LUA_MEMBER_FUNC(GetFloat)
            LUA_MEMBER_FUNC(IsNull)
            LUA_MEMBER_FUNC(IsObject)
            LUA_MEMBER_FUNC(IsArray)
            LUA_MEMBER_FUNC(Size)
        );
    }
    ...
    {
        using LUA_THIS = LANDiscoveryManager;
        LUA_CLASS(LANDiscoveryManager,
            sol::call_constructor, sol::factories([context]() {
                return SharedPtr<LANDiscoveryManager>(new LANDiscoveryManager(context));
            })
            LUA_BASES(Object)
            LUA_MEMBER_FUNC_RAW(Start, [](LANDiscoveryManager* manager, unsigned short port) {
                return manager && manager->Start(port);
            })
            LUA_MEMBER_FUNC(Stop)
            LUA_MEMBER_FUNC_RAW(SetBroadcastData, [](LANDiscoveryManager* manager, sol::table data, sol::this_state s) {
                if (!manager)
                    return;
                VariantMap map;
            ...
            LUA_MEMBER_FUNC(SetBroadcastTimeMs)
            LUA_MEMBER_FUNC(GetBroadcastTimeMs)
        );
    }
