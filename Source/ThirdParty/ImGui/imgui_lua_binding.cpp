...
static int lshowDockSpace(lua_State * L) {
    ...
    if (lua_getfield(L, 1, "update_slot") == LUA_TFUNCTION) {
        lua_pushvalue(L, 1);
        lua_pcall(L, 1, 1, 0);
    }
    ImGui::End();
...
static int
lisKeyPressedMap(lua_State* L) {
    int key = (int)luaL_checkinteger(L, 1);
    bool pressed = ImGui::IsKeyPressedMap(key);
...
static int wImage(lua_State *L) {
    ...
    bool is_gl_id = lua_toboolean(L, 5) != 0;
	ImTextureID tex_id = is_gl_id ? (ImTextureID)(intptr_t)lua_handle :luahandle_to_texture_id(L, lua_handle);
...
static int
wDockSpace(lua_State* L) {
    const char * label = luaL_checkstring(L, INDEX_ID);
    float width = (float)luaL_optnumber(L, 2, 0);
    float height = (float)luaL_optnumber(L, 3, 0);
    ImGuiDockNodeFlags flags = (ImGuiDockNodeFlags)luaL_optinteger(L, 4, 0);
    ImGui::DockSpace(ImGui::GetID(label), ImVec2(width, height), flags);
    return 0;
}
...
cGetColumnWidth(lua_State* L) {
    int index = (int)luaL_optinteger(L, 1, 0) - 1;
    float w = ImGui::GetColumnWidth(index);
    lua_pushnumber(L, w);
    return 1;
}
...
static struct enum_pair eImGuiKey[] = {
	ENUM(ImGuiKey,Tab),
...
static int
ugetTexGLID(lua_State* L)
{
    int lua_handle = (int)luaL_checkinteger(L, 1);
    ImTextureID tex_id = luahandle_to_texture_id(L, lua_handle);
    lua_pushinteger(L, (lua_Integer)tex_id);
    return 1;
}
...
luaopen_imgui(lua_State *L) {
	...
	luaL_Reg l[] = {
		...
		{ "end_frame", lendFrame },
		{ "keyboard", lkeyboard },
        { "isKeyPressedMap", lisKeyPressedMap },
        ...
        { "showDemoWindow", lshowDemoWindow },
